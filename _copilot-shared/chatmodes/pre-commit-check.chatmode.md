...
| 3 | mypy | |
| 4 | bandit | |
| 5 | secrets gate (`secrets_gate.py`) | |
| 6 | pytest + coverage | |
| 7 | markdownlint | |

Step 5 runs `python secrets_gate.py`. If you ever see the gate running
`detect_secrets scan --baseline` instead, treat that as a FAIL and report it:
that command writes newly found secrets *into* the baseline and exits `0`, so
it reports success while disarming the check.
...
The CI runner is Ubuntu, and the file list comes from `git ls-files`, which
always emits POSIX paths (`/`). If the baseline records Windows backslashes
(`\\`), the lookup misses and every recorded secret is reported as new.

On Windows this is hidden: `detect-secrets` converts POSIX paths to backslashes
locally, so the entries still match and the gate passes. The mismatch appears
only on Linux - so `sanity.bat` can be green while CI fails.

**This check is load-bearing.** The former step 5 (`detect_secrets scan
--baseline`) rewrote the baseline on every run, so a backslash baseline
silently self-corrected and this check never mattered. `secrets_gate.py` never
rewrites the file, so the mismatch now surfaces as a CI failure. That is the
correct behaviour - a gate should fail loudly rather than quietly repair
itself - but it means a baseline generated on Windows **must** be normalised
