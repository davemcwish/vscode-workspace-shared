# Beginner's Guide - Testing the Agency ZIP Script (Quote PDFs)

This guide explains how the `test_create_agency_zips_quote.py` test suite works,
...
This file contains **automated tests** that verify the `create_agency_zips_quote.py`
...
```bash
pytest tests/test_create_agency_zips_quote.py -v
```

### Run a Single Test Class

```bash
pytest tests/test_create_agency_zips_quote.py::TestParseFolderName -v
```

### Run a Single Test

```bash
pytest tests/test_create_agency_zips_quote.py::TestParseFolderName::test_valid_simple_name -v
```

### Run With Coverage

```bash
pytest tests/test_create_agency_zips_quote.py --cov=scripts --cov-report=term-missing
```
...
| `TestTimedDecorator` | `@timed` | Decorator preserves return value and logs timing. |
| `TestTimerContextManager` | `timer()` | Context manager logs label and elapsed time. |
| `TestLogSummary` | `log_summary()` | Outputs key metrics and compression info. |
...
```bash
pytest tests/test_create_agency_zips_quote.py::<YourTestClass>::<your_test> -v
```
...
```text
scripts/create_agency_zips_quote.py
```
