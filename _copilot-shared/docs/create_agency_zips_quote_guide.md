# Beginner's Guide - Create Agency ZIP Archives (Quote PDFs)

This guide explains how the `create_agency_zips_quote.py` script works, what you
...
After the `export_quote_pdfs.py` script downloads thousands of individual Quote
...
1. Scans the downloaded Quote PDF folder structure.
...
| Completed PDF export | You must have already run `export_quote_pdfs.py` to download the PDFs. |
...
were previously downloaded by `export_quote_pdfs.py`. However, if you need to
re-run the export first, see the
[Export Quote PDFs Guide](./export_quote_pdfs_guide.md) for Salesforce CLI setup
instructions.
...
| --- | --- | --- | --- |
| `--source-dir` | ✅ Yes | - | Folder containing the exported Quote PDF agency subfolders. |
...
```bash
python scripts/create_agency_zips_quote.py --source-dir "C:\Users\<you>\Downloads\AXP_Quote_PDFs_Prod_2026.05.27"
```
...
python scripts/create_agency_zips_quote.py ^
    --source-dir "C:\Users\<you>\Downloads\AXP_Quote_PDFs_Prod_2026.05.27" ^
    --output-dir "C:\Users\<you>\Downloads\AXP_Quote_Zips" ^
    --compression 6
```
...
1. Scan the `--source-dir` for Quote PDF subfolders.
...
| `DEFAULT_SOURCE_MANIFEST_FILE_NAME` | Filename the script looks for inside `--source-dir` when `--source-manifest` is not given | `export_quote_pdf_manifest_prod.csv` |
...
Misker Emmen.zip/
+-- 00000124_Misker Emmen_a0A8d00000DK2smEAD/
|   +-- Q-12345_Quote Name_QuoteCustomPDF.pdf
+-- 00000258_Misker Emmen_a0AJw000000uWq1MAE/
    +-- Q-67890_Another Quote_QuoteCustomPDF.pdf
```
...
```text
AXP_Quote_PDFs_Zipped_2026.05.19/
+-- Misker Emmen.zip
+-- Zeeuw & Zeeuw Utrecht.zip
+-- Hedin Automotive - Hoofddorp.zip
+-- ...
+-- create_quote_zips_manifest_2026.05.19.csv
+-- create_quote_zips_2026.05.19.log
+-- export_quote_manifest_prod_with_zips_2026.05.19.csv   <- enriched manifest
```
...
Ensure `SOURCE_DIR` points to the folder created by `export_quote_pdfs.py`.
The default expects:

```text
C:\Users\dwishar1\Downloads\AXP Decom 2026\AXP_Quote_PDFs_Prod
```
...
| **Visualforce** | A Salesforce framework for rendering custom pages; used to generate Quote PDFs. |
