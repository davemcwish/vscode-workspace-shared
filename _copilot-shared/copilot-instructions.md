...
Before committing to an implementation plan, assess confidence on a 0â€“100 scale:

| Confidence | Strategy |
| --- | --- |
| **High (>85%)** | Proceed with full implementation plan. |
| **Medium (66â€“85%)** | Build a proof-of-concept or MVP first. Define success criteria, validate, then expand. |
...
complete implementation costs minutes more than the shortcut â€” do the complete
thing. Every time.

- A "lake" is boilable â€” full test coverage for a module, all edge cases,
  complete error paths. An "ocean" is not â€” rewriting an entire system from
  scratch.
- When evaluating "approach A (full) vs approach B (90%)" â€” prefer A. The
...
Before building anything involving unfamiliar patterns â€” stop and search first.
The cost of checking is near-zero. The cost of not checking is reinventing
something worse.

Three layers of knowledge:

1. **Tried and true** â€” standard patterns, battle-tested approaches. Check
   whether the runtime or standard library already provides it.
2. **New and popular** â€” current best practices from documentation and
   ecosystem. Scrutinise what you find â€” the crowd can be wrong.
3. **First principles** â€” original reasoning about the specific problem. The
...
- When recommending a direction change â€” present the recommendation, explain
...
- Batch related edits into single operations. Don't make 5 edits when 1
  handles it.
- Skip filler confirmations like "I'll continue..." â€” just do it.
...
- Use **descriptive, meaningful names** â€” a reader should understand a variable's
  purpose without needing to look elsewhere.
- Prefer **explicit code over implicit behavior** â€” avoid hidden side effects,
  overly dynamic patterns, or "magic" unless clearly justified.

### Functions

- Keep functions **small and focused** â€” each does one clear thing.
- If a function is hard to explain in one sentence, split it into helpers.
- Add **type hints** for all parameters and return types.
- Write **complete-beginner docstrings** â€” assume the reader has never used
...
- Use **named constants** for repeated fixed values â€” no unexplained numbers or
  strings scattered through the code.
- Never hard-code **passwords, tokens, API keys, or secrets** â€” use environment
...
- Handle exceptions **gracefully and intentionally** â€” don't silently swallow errors.
...
- Use the **`logging` module** â€” never `print()` in production code.
...
- Use **virtual environments** â€” each project gets its own isolated environment.
...
- **No duplicated code** â€” extract reusable functions, but don't over-engineer early.
- **No obfuscation** â€” optimize for readability before cleverness.
- A **complete beginner** should be able to: read the code, understand its
  purpose, run the tests, and safely make changes â€” without needing to ask
...
| CI/CD | GitHub Actions â€” `ubuntu-latest` (Linux) |
| Security scanner | Cycode â€” SAST + secrets + SCA; runs on Linux on every PR |
...
| Rule | Why |
| --- | --- |
| Use `pathlib.Path` or `os.path.join()` â€” never backslash string literals | Backslash path separators fail on Linux |
...
| Step | Tool | Purpose |
| --- | --- | --- |
| 0 | `backlog-gate.chatmode.md` | Confirm the idea is not already in Â§8.4â€“Â§8.6. |
...
| Level | Label | Action |
| --- | --- | --- |
| ðŸ”´ | **CRITICAL** | Blocks merge. Security, correctness, data loss, breaking changes. |
| ðŸŸ¡ | **IMPORTANT** | Requires discussion. SOLID violations, missing tests, performance, architecture drift. |
| ðŸŸ¢ | **SUGGESTION** | Non-blocking. Readability, naming, minor optimisations, documentation gaps. |


##  Canonical Quality Gate

The single source of truth is `.github/workflows/ci.yml` (remote, blocks merge).
`sanity.bat` is the **local mirror** of that pipeline â€” run it before every
...
`pytest` invocation â€” `ci.yml`, `sanity.bat`, and plain `pytest` at the
...
vulnerable dependencies (SCA). Cycode runs on Linux â€” findings that only
reproduce on Linux can still block merge. Any Cycode finding is ðŸ”´ CRITICAL.
...
secrets) but is not a guarantee â€” Cycode has additional rules.
