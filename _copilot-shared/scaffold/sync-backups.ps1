...
function Get-GitSha {
    ...
    # when the ref is missing, so we can detect that cleanly. The "--" terminator
    # tells git that $Ref is a ref name and not an option flag, preventing
    # option-injection if $Ref begins with "-". stderr is redirected to $null so
    # a missing ref stays quiet -- we intentionally treat "not found" as a normal
    # $null result rather than an error to print.
