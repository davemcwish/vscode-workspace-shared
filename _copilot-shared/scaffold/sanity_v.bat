@echo off
REM ============================================================================
REM  sanity_v.bat - Run all code quality checks with verbose output
...
REM
REM  You must sanity.bat in sync with this file (same steps, verbose flags removed).
REM ============================================================================
REM
...
if not defined PY_TARGETS (
    ...
    %PY_CMD% -m ruff check %PY_TARGETS% --show-fixes
    if errorlevel 1 set /a FAIL_COUNT+=1
...
if %HAS_PYPROJECT% EQU 1 (
    ...
    %PY_CMD% -m mypy --show-error-codes --show-column-numbers --pretty
    if errorlevel 1 set /a FAIL_COUNT+=1
) else if defined PY_TARGETS (
    REM No pyproject.toml - pass targets directly with relaxed settings.
    %PY_CMD% -m mypy %PY_TARGETS% --ignore-missing-imports --show-error-codes --show-column-numbers --pretty
    if errorlevel 1 set /a FAIL_COUNT+=1
) else (
...
if not defined TEST_TARGETS (
    ...
    %PY_CMD% -m pytest %TEST_TARGETS% -n auto -v -ra --no-cov --override-ini="addopts="
    if errorlevel 1 set /a FAIL_COUNT+=1
) else (
    ...
    %PY_CMD% -m pytest %TEST_TARGETS% -n auto -v -ra
    if errorlevel 1 set /a FAIL_COUNT+=1
)
